...
function DifficultyDot({ diff }: { diff: number }) {
...
function PlayerCard({ player, isGanga }: { player: PlayerCard; isGanga?: boolean }) {
  ...
  return (
    <div style={{
      padding: '14px 16px',
      borderRadius: 12,
      background: isGanga ? 'rgba(34,197,94,0.08)' : 'rgba(255,255,255,0.04)',
      border: isGanga ? '1px solid rgba(34,197,94,0.25)' : '1px solid rgba(255,255,255,0.07)',
      display: 'flex',
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '36px 20px' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ color: '#9ca3af', fontSize: 15, margin: 0 }}>
            Gangas y mejores opciones por equipo analizadas por la IA
            {data?.totalPlayers ? ` · ${data.totalPlayers} jugadores analizados` : ''}
          </p>
        </div>
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '60px 0', color: '#6b7280' }}>
            ...
            <div>Analizando toda LaLiga...</div>
          </div>
        )}
          ...
          <>
            ...
            <section style={{ marginBottom: 48 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 14 }}>
                {data.gangas.map((p) => (
                  <PlayerCard key={p.name} player={p} isGanga />
                ))}
              </div>
            </section>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 28 }}>
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 8,
                    border: 'none',
                    background: selectedTeam === null ? '#16a34a' : 'rgba(255,255,255,0.06)',
                    color: selectedTeam === null ? 'white' : '#9ca3af',
                    fontWeight: 600,
                    fontSize: 13,
                    cursor: 'pointer',
                  }}
                ...
                {teams.map((team) => (
                  <button
                    ...
                    style={{
                      padding: '7px 14px',
                      borderRadius: 8,
                      border: selectedTeam === team ? '1px solid rgba(59,130,246,0.5)' : '1px solid rgba(255,255,255,0.08)',
                      background: selectedTeam === team ? 'rgba(59,130,246,0.15)' : 'rgba(255,255,255,0.04)',
                      color: selectedTeam === team ? '#93c5fd' : '#d1d5db',
              ...
              {selectedTeam ? (
                <div>
                  <h3 style={{ margin: '0 0 16px 0', fontSize: 18, fontWeight: 800, color: '#93c5fd' }}>
                    {selectedTeam} — Top jugadores esta jornada
                  </h3>
